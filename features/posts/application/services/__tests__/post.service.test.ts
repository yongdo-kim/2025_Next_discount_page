...
describe("PostService", () => {
  ...
  describe("getPostPreviews", () => {
    ...
    it("커서 기반 페이지네이션이 올바르게 작동해야 한다", async () => {
      ...
      const firstPage = await postService.getPostPreviews({
        req: firstPageReq,
      });
      ...
      const secondPage = await postService.getPostPreviews({
        req: secondPageReq,
      });
  ...
  describe("getPostDetail", () => {
    ...
    it("존재하지 않는 ID로 요청 시 에러를 발생시켜야 한다", async () => {
      ...
      await expect(postService.getPostDetail(invalidPostId)).rejects.toThrow(
        "Post not found",
      );
    });
  });
